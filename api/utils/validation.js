...
const validationUpdateProfile = (req) => {
   try{
   const allowedFieldsData = ["firstName","lastName","gender","age","about","genderPreference","photoUrl","skills"];
   const {firstName,lastName,emailId,password,skills,genderPreference,about,photoUrl} = req?.body;
   if(firstName?.length > 15 || lastName?.length > 15){
      throw new Error("Your FisrtName & LastName should be under 15 charcters");
   }else if(about?.length > 120){
